...
restart_logger = logging.getLogger(__name__)
restart_logger.setLevel(logging.DEBUG)
restart_fh = logging.FileHandler(f'{logdir}/restart.log')
restart_fh.setLevel(logging.DEBUG)
restart_fh.setFormatter(logging.Formatter('%(name)s:%(levelname)s:%(asctime)s:%(message)s'))
restart_logger.addHandler(restart_fh)
restart_logger.info(f'Execution parameters: {sys.argv}')
...
def generator(threadID, queue: tmp.Queue, modelg, model_B, slow_control: tmp.Value, queue_B: tmp.Queue = None):
    ...
    if not training_mod.get("state_percentage"):
        state_percentage = 1.0
    else:
        state_percentage = training_mod.get("state_percentage")
    ...
    while True:
        ...
        for restartcount in range(10000):
            max_tile = np.max(state.board)
            restart_logger.info(
                f"start_max_tile:{max_tile} start_score:{state.score}"
            )
            lastboard = None
            lastnode = None
            ...
            while True:
                ...
                if state.isGameOver():
                    ...
                    max_tile = np.max(state.board)
                    restart_logger.info(
                        f"end_max_tile:{max_tile} end_score:{state.score}"
                    )
                    break
